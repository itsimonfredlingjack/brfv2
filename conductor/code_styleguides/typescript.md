...
For Träff TypeScript clients:

- **Mobile PWA (`xs_mobilapp/`):** oxlint + `tsconfig.json` (`npm run lint`, typecheck scripts as defined)
- **Native (`kalla-native/`):** ESLint (`eslint.config.js`) + Expo/TS configs
- No root Prettier policy unless the project explicitly adopts one later

The guide below is a baseline; **repo lint and local conventions win** on conflicts.
