...
For Träff backend Python:

- **Config:** root `ruff.toml` (Python 3.12 target)
- **Runner:** `uvx ruff@0.15.0 check --config ruff.toml <file>` — do **not** put ruff in `backend/pyproject.toml` (desktop artifact identity hash)
- Prefer existing patterns in `backend/app/` over inventing new frameworks

The guide below is a baseline; **repo lint config wins** on conflicts.
