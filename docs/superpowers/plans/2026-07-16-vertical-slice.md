...
- [ ] **Task 0 — Housekeeping & baseline.** Extend `.gitignore` (secrets/data/scratch); commit
  the pre-existing WIP UI as a baseline commit; commit SPEC.md + this plan.
- [ ] **Task 1 — Backend scaffold.** `backend/pyproject.toml` (uv, py3.12; deps: fastapi,
...
- [ ] **Task 2 — normalize.py (TDD).** Tests first: NFKC/ligature (`ﬁn`→`fin`), NBSP/thin-space,
...
- [ ] **Task 3 — extract.py (TDD).** Fixture builder `tests/pdf_fixtures.py::build_pdf(pages:
...
- [ ] **Task 4 — chunker.py (TDD).** `chunk_pages(pages, strategy, size, overlap) -> list[Chunk]`
...
- [ ] **Task 5 — embeddings.py + indexer.py (TDD).** `HashedTfidfEmbedder` (char 3–5-gram
...
- [ ] **Task 6 — citations.py (TDD — the core).** Tests per SPEC §2 using crafted fixture PDFs:
...
- [ ] **Task 7 — llm.py.** `parse_llm_json` TDD (clean JSON, fenced, prose-wrapped, invalid →
...
- [ ] **Task 8 — answer.py + /api/ask (TDD w/ FakeLLM).** Gates: empty index → refusal
  ...
  refusal. Commit.
- [ ] **Task 9 — store.py + remaining API.** Disk layout `backend/data/{documents.json,
...
- [ ] **Task 10 — seed corpus + golden set.** `scripts/seed_content.py`: 5 synthetic Swedish
...
- [ ] **Task 11 — frontend wiring: api.js, upload, documents.** Vite proxy `/api`→8787.
...
- [ ] **Task 12 — frontend: chat + PdfViewer highlights.** Chat calls `/api/ask`; renders
...
- [ ] **Task 13 — FLOOR PROOF.** Seed; run backend+frontend; through the browser (Chrome MCP or
...
- [ ] **Task 14 — Eval harness (item 1).** `scripts/eval.py`: loads golden.json, runs
...
- [ ] **Task 15 — Failure-mode hardening (item 2).** Audit §2 ↔ tests matrix; add adversarial
...
- [ ] **Task 16 — Settings wiring proof (item 3).** `scripts/eval.py --sweep`: runs eval
...
- [ ] **Task 17 — Multi-document proof (item 4).** Golden set already cross-doc; add eval
...
- [ ] **Task 18 — Demo polish (item 5).** `DEMO.md` (scripted flow, exact questions, reset
...
- [ ] **Task 19 — OCR spike rig (item 6).** `scripts/ocr_spike.py` per SPEC §5 with
