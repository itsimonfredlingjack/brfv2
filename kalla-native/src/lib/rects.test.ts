...
/* The transform that replaces a PDF-rendering engine. If it drifts, every
 * highlight in the product lands somewhere other than the passage it is
 * claiming to prove — which is worse than showing no highlight at all.
 * Ported from xs_mobilapp/src/lib/rects.test.ts; boxes are fractions (0..1)
 * here instead of CSS percentage strings, since the native overlay math
 * needs numbers to multiply against a measured page size. */

// A4 at PyMuPDF's default: exactly what the seeded corpus renders as.
const A4 = { width: 595, height: 842 }

describe('rectToBox', () => {
  it('maps a rect to fractions of the page', () => {
    const box = rectToBox([72, 100, 300, 114], A4)!
    expect(box).not.toBeNull()
    expect(box.left).toBeCloseTo(72 / 595, 5)
    expect(box.top).toBeCloseTo(100 / 842, 5)
    expect(box.width).toBeCloseTo(228 / 595, 5)
    expect(box.height).toBeCloseTo(14 / 842, 5)
  })

  it('does NOT flip the y axis', () => {
    // Backend rects are top-left origin (app/extract.py). A rect near the
    // top of the page must land near the top of the image.
    const box = rectToBox([50, 40, 200, 60], A4)!
    expect(box.top).toBeLessThan(0.1)
  })

  it('places a rect near the bottom of the page near the bottom of the image', () => {
    const box = rectToBox([50, 800, 200, 820], A4)!
    expect(box.top).toBeGreaterThan(0.94)
  })
