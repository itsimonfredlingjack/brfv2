...
# --------------------------------------------------------------------------
# The session transport, on the desktop routes
#
# The desktop adapter gives each installation its own cookie *name* and scopes
# it to /api/.  It does not give the desktop routes their own way to present a
# session: the httpOnly cookie is the only transport here too.  These tests are
# the desktop half of the guarantee backend/tests/test_api.py holds for the web
# product — without them, the one place that mints a session without a password
# prompt (first-run provisioning) and the most privileged surface in the product
# (model service, backup, restore) would be the two least covered.
# --------------------------------------------------------------------------


def test_setup_does_not_echo_the_session_token(tmp_path):
    """First-run provisioning signs the owner in. It must not hand page script
    a long-lived credential while doing it."""
    with _client(tmp_path) as client:
        body = client.post("/api/desktop/setup", json=OWNER)
        assert body.status_code == 200, body.text
        payload = body.json()
        cookie_header = body.headers["set-cookie"]

    assert set(payload) == {"user", "memberships"}
    assert "token" not in payload
    # The credential exists — it is simply only in the httpOnly cookie.
...
def test_a_real_session_token_is_refused_as_a_bearer_header(tmp_path):
    """It is the *transport* that is rejected, not the credential.

    The same token that works in the cookie is offered in an Authorization
    header; a desktop route must not accept it. Proving both halves in one test
    is what distinguishes 'Bearer is not accepted' from 'that token was
    invalid anyway'.
    """
    with _provisioned(tmp_path) as client:
        cookie_name = next(name for name in client.cookies if name.startswith("brf_desktop_"))
        token = client.cookies[cookie_name]

        with_cookie = client.get("/api/desktop/model-runtime")
    ...
    assert as_bearer_state.status_code == 401


def test_login_through_the_desktop_boundary_is_cookie_only(tmp_path):
    """The product's own /api/auth/login, reached through the desktop origin,
    keeps the same contract and sets the installation's own cookie."""
    with _provisioned(tmp_path) as client:
        client.cookies.clear()
        response = client.post(
            "/api/auth/login",
    ...
    assert response.status_code == 200, response.text
    assert set(payload) == {"user", "memberships"}
    assert cookie_header.startswith("brf_desktop_")
    assert "path=/api/" in cookie_header.lower()
    # The default web cookie name is not also set: one installation, one cookie.
    assert "brf_session=" not in cookie_header
    assert me.status_code == 200


def test_the_installation_cookie_name_is_per_install_and_stable(tmp_path):
    """Two installations do not share a cookie name; one installation does not
    change its own across restarts (a changed name would silently sign every
    account out)."""
    first = _provisioned(tmp_path / "a")
    with first as client:
        name_a = next(n for n in client.cookies if n.startswith("brf_desktop_"))
    second = _provisioned(tmp_path / "b")
    with second as client:
        name_b = next(n for n in client.cookies if n.startswith("brf_desktop_"))

    assert name_a != name_b
    assert re.fullmatch(r"brf_desktop_[a-f0-9]{24}", name_a)

    # Same data root, second launch: the name is read back, not re-minted.
