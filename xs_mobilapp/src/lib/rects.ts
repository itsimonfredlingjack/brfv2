/* Citation rect → CSS box.
 *
 * This is the whole reason the app can skip pdf.js. The backend rasterizes a
 * page at a known pixel width; the citation rects are ALREADY top-left-origin
 * PDF points in that same coordinate space (app/extract.py reads
 * `page.rect` + `get_text("words")`, and `get_pixmap` renders the same view,
 * rotation included). So one uniform scale is the entire transform:
 *
 *   scale = renderedWidthPx / pageWidthPt
 *
 * No y-flip, no viewport matrix, no rotation handling — all of which the
 * desktop's PdfPane.jsx needs only because pdf.js works in y-UP user space.
 *
 * Expressed as percentages rather than pixels so the highlight stays glued to
 * the passage while the image reflows (rotation, tablet pane, zoom) without
 * re-measuring anything.
 */

export interface HighlightBox {
  /** CSS percentage strings, relative to the rendered page element. */
  left: string
  top: string
  width: string
  height: string
 ...
 * percentage-based CSS box. Returns null for a rect that is malformed or
 * entirely outside the page — the backend already rejects out-of-bounds
 * rects (SPEC §2.7), so this is defense in depth, and a bad rect must render
 * as *nothing* rather than as a box in the wrong place.
 */
export function rectToBox(rect: number[], page: PageSize): HighlightBox | null {
  ...
  return {
    left: pct(cl / page.width),
    top: pct(ct / page.height),
    width: pct((cr - cl) / page.width),
    height: pct((cb - ct) / page.height),
  }
}
...
/**
 * Pick the rasterization width to request for a given CSS width. The backend
 * allowlist is closed (720 / 1080 / 1440), so this picks the smallest width
 * that still covers the device's physical pixels — asking for 1440 on a
 * small phone wastes bandwidth the user is paying for.
 */
